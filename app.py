...
INSERT INTO users VALUES(null,"yoloAIS22", "klimavalg18DK");
...
def notes():
    ...
    if request.method == 'POST':
        if request.form['submit_button'] == 'add note':
            ...
            c.execute(statement, {'publicID': noteid})
            result = c.fetchall()
            if(len(result)>0):
                row = result[0]
                statement = """INSERT INTO notes(id,assocUser,dateWritten,note,publicID) VALUES(null,?,?,?,?)""" 
    ...
    print(statement, {'assocUser': session['userid']})
    c.execute(statement, {'assocUser': session['userid']})
    notes = c.fetchall()
    print(notes)
...
def register():
    ...
    if request.method == 'POST':
        ...
        if(len(c.fetchall())>0):
            errored = True
            passworderror = "Password or username is already in use"

        c.execute(user_statement, {'username': username})
        if(len(c.fetchall())>0):
            errored = True
            usererror = "Password or username is already in use"
